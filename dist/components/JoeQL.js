...
function transpile(query) {
  ...
  function traverseFields(selectionSet, wrap) {
    var fields = selectionSet.map(function (selection) {
      if (selection.selectionSet) return [selection.name, traverseFields(selection.selectionSet, true)];else return selection.name;
    });

    fields = fields.join(',');

    if (wrap) return ('(' + fields + ')').replace(',(', '(');else return fields.replace(',(', '(');
  }

  var result = slimTree.map(function (st) {
